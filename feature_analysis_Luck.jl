#= feature importance analysis: randomly remove features in combinations of 5 to 9 features. Dropping "smallest" column is the prefix.
	Julia v1.9.1
	MLJ v0.19.2
	DataFrames v1.6.1
        CSV v0.10.11
...
feature_num = 2 # starting number of removed features. We want to get big effect with as little features as possible
...
# "feature_matrix_F3-1.csv" is the data matrix containing target Psum and the features. Dropping "smallest" column from "feature_matrix_F3.csv" to make "feature_matrix_F3-1.csv". "feature_names.csv" is simply the column names of "feature_matrix_F3-1.csv"
df_origin = CSV.read("feature_matrix_F3-1.csv", DataFrame)
df_name = CSV.read("feature_names.csv", DataFrame)
col_names = df_name[!, 1]
...
@showprogress for i in 1:draw_num
       ...
       # record combinations that increase mse; if the new MSE is similar with previous value, the one with less features removal is kept.
